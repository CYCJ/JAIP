                ...
                int cls_gnum)
{
    ...
    *cp_nbytes = ((ref_field_cnt + ref_method_cnt + cls_cnt + ldc_cnt)<<2) + (ref_method_cnt<<1);
    ...
    if(temp1 != 999){
        ...
        for (idx = 0; idx < x_ref_tbl[temp1].field_cnt; idx++)
        {
            ...
            if((x_ref_tbl[temp1].field[idx].field_tag & 0x0800) == 0){
                // non-static field data
                if ((x_ref_tbl[temp1].field[idx].field_tag & 0x000F) == 0x0006){
                    // long
                    uint16_temp+=2;
                }
    ...
    for (idx = 0; idx < fd_cnt; idx++)
    {
        ...
        */
        ...
        x_ref_tbl[cls_gnum].field[jdx].cls_id = cls_gnum;
        x_ref_tbl[cls_gnum].field[jdx].field_tag = (access_flags<<8) | descriptor_flag;
    ...
    */
    ...
    for (idx = 0; idx < ref_field_cnt; idx++)
    {
        ...
        */
        ...
        if(jdx == global_num)
        {
			...
			ref_field_addr[idx] = x_ref_tbl[jdx].field[0].cls_id << 16 | ((uint)address & 0x0000FFFF);
			global_num ++;
        }
        else
        {
            ...
            if(kdx == uint16_temp)
            {
				...
				ref_field_addr[idx] = x_ref_tbl[jdx].field[uint16_temp].cls_id << 16 | ((uint)address & 0x0000FFFF);
                x_ref_tbl[jdx].field_cnt++;
            }
            else
            {
                address = x_ref_tbl[jdx].field[kdx].field_address;
				ref_field_addr[idx] = x_ref_tbl[jdx].field[kdx].cls_id << 16 | ((uint)address & 0x0000FFFF);
            }
        }
    ...
    for (idx = 0; idx < ldc_cnt; idx++)
    {
        ...
        case CP_Integer:
            /*
            printf("%x\n", *((uint8*)classfile + temp + 1));
            printf("%x\n", *((uint8*)classfile + temp + 2));
            printf("%x\n", *((uint8*)classfile + temp + 3));
            printf("%x\n", *((uint8*)classfile + temp + 4));
            */
