...
int initialize_heapmemory()
{
	...
	for(i=0;i<0x8000;i++){
	//for(i=0;i<0x10000;i++){
		memset( (uint *)(HEAP_MEM_AREA_PTR+i*(HEAP_MEM_AREA_MAX_SIZE>>2)) , 0x00000000, (HEAP_MEM_AREA_MAX_SIZE>>2));
	}
    
    xil_printf("\r\n-- Memory allocate 0x%8x as heap memory area , total %ld bytes ... ", HEAP_MEM_AREA_PTR, 0x8000*HEAP_MEM_AREA_MAX_SIZE);


    //2012 added by jing
    //memset((void *) METHOD_TABLE_ADDRESS , 0, MAX_METHOD_TABLE);
