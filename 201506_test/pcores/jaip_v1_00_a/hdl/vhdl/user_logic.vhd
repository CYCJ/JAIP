...
architecture IMP of user_logic is
  ...
  signal reg_A, reg_B, reg_C		: std_logic_vector(0 to 31);
 ...
 -- signal debug_in					: std_logic_vector(0 to 31);
	...
	component cross_reference_table is
	generic(
...
-- cache-PLB
	...
	signal debug_cs_soj				:  std_logic_vector(57 downto 0);
	signal debug_jcode_counter		: std_logic_vector(7 downto 0);
...
-- enable write back during ISR 
  ...
  begin
	...
	case mem_address is
		...
		when "00000000000010" => data_out <= reg_A(byte_index*8 to byte_index*8+7);
		when "00000000000011" => data_out <= reg_B(byte_index*8 to byte_index*8+7);
		when "00000000000100" => data_out <= reg_C(byte_index*8 to byte_index*8+7);
...
-- bytecode time profiler
   ...
   begin
	if(Bus2IP_Reset = '1') then
		...
		debug_instrs_pkg <= (others => '0');
		debug_CTRL_state <= (others => '0');
		debug_jpl_mst_address <= (others => '0');
		debug_CST_entry <= (others => '0');
		debug_external	<= (others => '0');
		
	elsif(rising_edge(Bus2IP_Clk)) then
		debug_TOS_A 	<= reg_A;
		debug_TOS_B 	<= reg_B;
		debug_TOS_C 	<= reg_C;
		debug_instrs_pkg<=debug_cs_soj(21 downto 6);
		debug_trig 		<= debug_jcode_counter;
		debug_CTRL_state <=debug_cs_soj(5 downto 0) ;
		debug_jpl_mst_address <= jpl_mst_address;
		debug_CST_entry <= debug_cs_soj(57 downto 26);
		debug_external	<= jpl_load_data;
