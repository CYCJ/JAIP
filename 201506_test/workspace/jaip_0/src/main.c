...
int main(void)
{
	...
	*JPL3_XCPTN_ENABLE_volatile = 0xF0000000;
	...
	if ((fptr = sysace_fopen("a:\\jar\\testAB.jar", "r")) == NULL)
	{
	...
	*COOR_CMD_MSG = (0x4<<29) | (0x3<<26) | (0x00000000);
	...
	for(i = 0; i < global_mthd_id ; ++i){
		*GET_CTRL_REG_MEM0ADDR(default_core_id, JPL_debug_addr ) = i << 8 | 0;
		*GET_CTRL_REG_MEM0ADDR(default_core_id, JPL_debug_addr ) = i << 8 | 0;
		*GET_CTRL_REG_MEM0ADDR(default_core_id, JPL_debug_addr ) = i << 8 | 0;
		HW_time_tmp = *GET_CTRL_REG_MEM0ADDR(default_core_id, JPL_MTHD_PROFILE );
		if (HW_time_tmp==0) continue;
		printf("Method #%X HW time:%u \r\n", i, HW_time_tmp);

		*GET_CTRL_REG_MEM0ADDR(default_core_id, JPL_debug_addr ) = i << 8 | 1;
		*GET_CTRL_REG_MEM0ADDR(default_core_id, JPL_debug_addr ) = i << 8 | 1;
		*GET_CTRL_REG_MEM0ADDR(default_core_id, JPL_debug_addr ) = i << 8 | 1;
		Intrpt_time_tmp = *GET_CTRL_REG_MEM0ADDR(default_core_id, JPL_MTHD_PROFILE );
		printf("Method #%X Intrpt time:%u \r\n", i, Intrpt_time_tmp);

		*GET_CTRL_REG_MEM0ADDR(default_core_id, JPL_debug_addr ) = i << 8 | 2;
		*GET_CTRL_REG_MEM0ADDR(default_core_id, JPL_debug_addr ) = i << 8 | 2;
		*GET_CTRL_REG_MEM0ADDR(default_core_id, JPL_debug_addr ) = i << 8 | 2;
		DR_time_tmp = *GET_CTRL_REG_MEM0ADDR(default_core_id, JPL_MTHD_PROFILE );
		printf("Method #%X DR time:%u \r\n", i, DR_time_tmp);

		*GET_CTRL_REG_MEM0ADDR(default_core_id, JPL_debug_addr ) = i << 8 | 3;
		*GET_CTRL_REG_MEM0ADDR(default_core_id, JPL_debug_addr ) = i << 8 | 3;
		*GET_CTRL_REG_MEM0ADDR(default_core_id, JPL_debug_addr ) = i << 8 | 3;
		Mem_time_tmp = *GET_CTRL_REG_MEM0ADDR(default_core_id, JPL_MTHD_PROFILE );
		printf("Method #%X Mem time:%u \r\n", i, Mem_time_tmp);

		*GET_CTRL_REG_MEM0ADDR(default_core_id, JPL_debug_addr ) = i << 8 | 4;
		*GET_CTRL_REG_MEM0ADDR(default_core_id, JPL_debug_addr ) = i << 8 | 4;
		*GET_CTRL_REG_MEM0ADDR(default_core_id, JPL_debug_addr ) = i << 8 | 4;
		Num_of_calls = *GET_CTRL_REG_MEM0ADDR(default_core_id, JPL_MTHD_PROFILE );
