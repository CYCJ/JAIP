...
uint num_entry_xrt_field_count;
uint num_entry_xrt_method_count;
...
void initialize_xrt_ref(){
	...
	for (i=0;i<MAX_CLASS_NUMBER;i++){
		x_ref_tbl[i].IsCache = 0;
	}
	global_num = 0;
	mt_table_count = 0;
	num_entry_xrt_field_count = 0;
	num_entry_xrt_method_count = 0;
}
...
// add for parsing Thread.start() , Thread.tun()
...
void update_cst_entry_for_native_method
	(uint16 cls_gnum, uint16 mtnamel, uint16 desnamel, uint8 *methodname, uint8 *desname, uint *address){
...
/* -------------------------------------------------------------- */
int parse_classfile(uint8	core_id,	uint8  *classfile	,	uint16 *parent_index,
				uint16 *CPcount,	uint16 *cp_toc		,	uint8  *cp_data,
				uint32 *cp_nbytes,	uint16 *fd_idx_img2cls, uint8  *fd_data,
				uint32 *fd_nbytes,	uint16 *mt_idx_img2cls, uint8  *mt_code,
				uint32 *mt_nbytes,	uint16 *main_pc,		uint16 *clinit_pc,
				uint8  *cls_name,	uint16 *main_locals,	int cls_gnum){
	...
	uint   temp1, temp, attribute_size, temp2;
	...
	// 0x200 it means that this class actually is an interface
	...
	*cp_nbytes = ((ref_field_cnt + ref_method_cnt + cls_cnt + ldc_cnt)<<2) + (ref_method_cnt<<1);
	...
	if(temp1 != 999){
		...
		for (idx = 0; idx < x_ref_tbl[temp1].field_cnt; idx++){
			...
			else address = x_ref_tbl[cls_gnum].field[jdx].field_address;
	...
	if(x_ref_tbl[cls_gnum].isIntf != 1){
		if(temp1 != 999){
			for (idx = 0; idx < x_ref_tbl[temp1].method_cnt; idx++){
				...
				// note, synchronized bit should be erased for new overriden  sync method
	...
	for (idx = 0; idx < mt_cnt; idx++){
		...
		if(aligment_shift!=0) mt_offset += 4 - aligment_shift;

		access_flags = read_uint16(cf_ptr);
		
		// access flag : static flag
		if ((access_flags & 0x0008) != 0) nonstatic = 0;
		else nonstatic = 1;
		...
		if((access_flags & 0x0400) != 0){
			...
			//  Thread.start() non-static native method.
			else if(mtnamel == 5 && desnamel == 3
					&& !strncmp(methodname, "start", mtnamel)
					&& !strncmp(desname, "()V", desnamel)
					&& !strncmp(x_ref_tbl[cls_gnum].class_name, "java/lang/Thread", 16)
		...
		*(tmp + 1) = method_descriptor_analyzer(desname) + nonstatic;
			...
			if (attrnamel == 4 && strncmp(attrname, "Code", 4) == 0){
				...
				for (edx = 0 ; edx < ER_cnt ; edx++){	
					...
					for (fdx = 0; fdx < global_num; fdx++)
						if (x_ref_tbl[fdx].class_namel == cnamel && !(strncmp(x_ref_tbl[fdx].class_name, classname, cnamel))) break;
				...
				printf("The attribute \"%X\" (%s) has not defined, attribute length: %u\n", attrname, attrname, attrnamel);
				system("PAUSE");
			}
		}
		...
		for(core_idx=0; core_idx< JAIP_CORE_NUM; core_idx++){
			...
			*JPL_MA_LUT_REG_volatile =	(((unsigned int) global_mthd_id << 2) + 2) << 16 | ((code_size+8));
			*JPL_MA_LUT_REG_volatile = 0xFFFF0000;
		}
	...
	*mt_nbytes = mt_offset;
	...
	for (idx = 0; idx < ref_method_cnt; idx++){
		...
		if (x_ref_tbl[cls_gnum].class_namel == cnamel
			&& !(strncmp(x_ref_tbl[cls_gnum].class_name, classname, cnamel))) jdx = cls_gnum;
		else
			for (jdx = 0; jdx < global_num; jdx++)
				if (x_ref_tbl[jdx].class_namel == cnamel
					&& !(strncmp(x_ref_tbl[jdx].class_name, classname, cnamel))) break;
		...
		if(jdx == global_num){
			...
			if (tag != 0xB){
				...
				for(core_idx=0; core_idx<JAIP_CORE_NUM; core_idx++) *GET_LV1_XRT_ABS_ADDR(core_idx, address) = 0x8000FFFF;
			}
		...
		for(idx=0; idx<x_ref_tbl[cls_gnum].method_cnt ; idx++ ){
			...
			if(mtnamel == 3 && desnamel == 3
				...
				run_method_global_mthd_idx =
						*GET_LV1_XRT_ABS_ADDR(core_id, x_ref_tbl[cls_gnum].method[idx].method_offset + (1<<2)) & 0x00000FFF;
			}
			else if(mtnamel == 5 && desnamel == 3
					&& !strncmp(methodname, "start", mtnamel)
					&& !strncmp(desname, "()V", desnamel)
	...
	// add method offset to interface's and super class's method list if
	...
	if(x_ref_tbl[cls_gnum].isIntf != 1){
		for (idx = 0; idx < x_ref_tbl[cls_gnum].method_cnt; idx++){
			...
			// add method to interface and super class's method list
			...
			for (jdx = 0; jdx < x_ref_tbl[cls_gnum].intf_cnt; jdx++){
				...
				//search interface's method list's last node
			...
			while(uint16_temp!= 999){
				...
				if(zdx == x_ref_tbl[uint16_temp].method_cnt) break;
				
				//search super class's method list's last node
				address = x_ref_tbl[uint16_temp].method[zdx].method_offset;
				while( (*GET_LV1_XRT_ABS_ADDR(core_id, address) & 0xFFFF) != 0xFFFF){
					address = *GET_LV1_XRT_ABS_ADDR(core_id, address)>>16;
				}
	...
	if( mt_table_count > ((0x1000)<<0) ){
		printf("[class_loader.c] line 1979,  while class=%s , XRT size usage: %x \n",
	...
	if (x_ref_tbl[cls_gnum].class_namel < 3){
		xil_printf("class_name:%s\r\n",x_ref_tbl[cls_gnum].class_name);
		xil_printf("base_address:%x\r\n",x_ref_tbl[cls_gnum].base_address);
		xil_printf("parent_index:%d\r\n",x_ref_tbl[cls_gnum].parent_index);
		xil_printf("method_cnt:%d\r\n",x_ref_tbl[cls_gnum].method_cnt);
		xil_printf("field_cnt:%d\r\n",x_ref_tbl[cls_gnum].field_cnt);
		xil_printf("intf_cnt:%d\r\n",x_ref_tbl[cls_gnum].intf_cnt);
		xil_printf("obj_size:%x\r\n",x_ref_tbl[cls_gnum].obj_size);
		for (idx = 0; idx < x_ref_tbl[cls_gnum].method_cnt; idx++){
			xil_printf("method #%2d:%s\r\n", idx, x_ref_tbl[cls_gnum].method[idx].method_name);
			xil_printf("method_offset:%x\r\n",x_ref_tbl[cls_gnum].method[idx].method_offset);
			xil_printf("arg_size:%x\r\n",x_ref_tbl[cls_gnum].method[idx].arg_size);
			xil_printf("GET_LV1_XRT_ABS_ADDR(core_idx, address):%x\r\n",GET_LV1_XRT_ABS_ADDR(core_idx, x_ref_tbl[cls_gnum].method[idx].method_offset));
			xil_printf("*GET_LV1_XRT_ABS_ADDR(core_idx, address):%x\r\n",*GET_LV1_XRT_ABS_ADDR(core_idx, x_ref_tbl[cls_gnum].method[idx].method_offset));
			xil_printf("GET_LV1_XRT_ABS_ADDR(core_idx, address+4):%x\r\n",GET_LV1_XRT_ABS_ADDR(core_idx, x_ref_tbl[cls_gnum].method[idx].method_offset+4));
			xil_printf("*GET_LV1_XRT_ABS_ADDR(core_idx, address+4):%x\r\n",*GET_LV1_XRT_ABS_ADDR(core_idx, x_ref_tbl[cls_gnum].method[idx].method_offset+4));
		}
	}
	...
	for (idx = 0; idx < ldc_cnt; idx++){
		...
		case CP_String:
			...
			*(unsigned int*)CURRENT_HEAP_PTR = uint16_temp2;
			...
			char * utf8_ptr = (char *)(classfile + uint16_temp +3);
			for(jdx = 0; jdx < uint16_temp2;jdx++){
				((unsigned short *)CURRENT_HEAP_PTR)[jdx] = utf2unicode(&utf8_ptr);
			}

			if((uint16_temp2 & 0x00000001) != 0){
				CURRENT_HEAP_PTR = CURRENT_HEAP_PTR + ((uint16_temp2<<1)&0xFFFFFFFC) + 4;
			}
			else CURRENT_HEAP_PTR = CURRENT_HEAP_PTR + (uint16_temp2<<1);
			heap_align32();
			break;
		case CP_Float:
		case CP_Integer:
	...
	x_ref_tbl[cls_gnum].cst_size = 4 + (nitems<<1) + *cp_nbytes;
...
/* ------------------------------------------------------------------------- */
int clinit_initializer(uint16 *cp_toc, uint8 *cp_data,
